defmodule CritistryWeb.CritSessionController do
  ...
  def create(conn, %{"id" => id, "crit_session" => %{"image" => crit_session_image} = crit_session_params} = params, user) do  
    IO.inspect params
    conn
    |> create_crit_session(user, id, %{crit_session_params | "image" => Upload.upload_image(crit_session_image)})
  end
