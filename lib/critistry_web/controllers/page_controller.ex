defmodule CritistryWeb.PageController do
  ...
  def index(conn, _params) do
    ...
    render(conn, "index.html",
      stats: %{
        ...
        crit_group_count: Crits.get_crit_group_count(),
        crit_session_count: Crits.get_crit_session_count(),
        crit_count: Crits.get_crit_count()
      }
    )
  end
  ...
  defp setup_crit_sessions(crit_session_factor) do
    Auth.list_users()
    |> Enum.each(fn u ->
      setup_user_crit_sessions(
        u,
        Enum.take_random(1..100, Enum.random(1..6)),
        crit_session_factor
      )
    end)
  end
  ...
  defp setup_crit(user_id, crit_factor) do
    ...
    query =
      from(c in "users_crit_groups",
        join: cs in CritSession,
        on: cs.crit_group_id == c.crit_group_id,
    ...
    |> Enum.each(fn id ->
      Crits.setup_crit(id, Auth.get_user!(user_id), Enum.random(crit_sessions))
    end)
  end
end
